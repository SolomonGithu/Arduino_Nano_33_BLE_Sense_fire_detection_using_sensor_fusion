...
#include <Sensor_Fusion_inferencing.h>
...
void setup() {
  ...
  Serial.println("Arduino Nano 33 BLE Sense image and temperature data collection");

  if (!init_HTS221()) {
    Serial.println("HTS221 sensor initialization failed!");
  }
...
void loop() {
  ...
  if (ei_camera_init() == false) {
    Serial.println("ERR: Failed to initialize image sensor");
  }
  ...
  if (res) {
    Serial.print("ERR: Failed to calculate resize dimensions");
    Serial.println(res);
  }
  ...
  if (snapshot_mem == NULL) {
    Serial.println("failed to create snapshot_mem");
  }
  ...
  if (!signal_buf) {
    Serial.println("ERR: Failed to allocate signal buffer");
  ...
  if (!per_pixel_buffer) {
    ei_free(signal_buf);
    ei_free(snapshot_mem);
    Serial.println("ERR: Failed to allocate per_pixel buffer");
  }
  ...
  for (size_t ix = 0; ix < signal.total_length; ix += signal_chunk_size) {
    ...
    if (r != 0) {
      Serial.print("ERR: Failed to get data from signal ");
      Serial.println(r);
      ei_free(signal_buf);
    ...
    for (size_t px = 0; px < items_to_read; px++) {
      ...
      if (per_pixel_buffer_ix >= 513) {
        ...
        if (!base64_buffer) {
          Serial.print("ERR: Cannot allocate base64 buffer of size ");
          Serial.print(base64_output_size);
        ...
        if (r < 0) {
          Serial.print("ERR: Failed to base64 encode ");
          Serial.println(r);
          ei_free(signal_buf);
          ei_free(per_pixel_buffer);
          ei_free(snapshot_mem);
  ...
  if (!base64_buffer) {
    Serial.print("ERR: Cannot allocate base64 buffer of size ");
    Serial.print(new_base64_buffer_output_size);
    Serial.println(" out of memory");
    ei_free(signal_buf);
    ei_free(per_pixel_buffer);
    ei_free(snapshot_mem);
  ...
  // Wait for the Python script to save an .jpg image
  delay(10000);
}
...
bool ei_camera_capture(uint32_t img_width, uint32_t img_height, uint8_t *out_buf)
{
  ...
  if (do_crop) {
    ...
    //Serial.print("crop cols: ");
    //Serial.print(crop_col_sz);
    //Serial.print(" rows: );
    //Serial.println(crop_row_sz);
...
int OV7675::allocate_scratch_buffs()
{
  ...
  if (buf_mem == NULL) {
    //Serial.println("failed to create buf_mem");
    return false;
  }
  raw_buf = (uint8_t *)DWORD_ALIGN_PTR((uintptr_t)buf_mem);

  //Serial.println("allocating buffers OK");
  return 0;
}

int OV7675::deallocate_scratch_buffs()
{
  //Serial.println("deallocating buffers...");
  ei_free(buf_mem);
  buf_mem = NULL;

  //Serial.println("deallocating buffers OK");
  return 0;
}
